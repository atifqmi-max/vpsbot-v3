...
import os, io, time, socket, random, string, tarfile, asyncio, logging, sqlite3, datetime
        ...
        c.execute("""
            INSERT INTO vps
              (vps_id,owner_id,container_id,os_image,os_label,
               ...
               ssh_ip,ssh_port,root_pass,username,status,expires_at)
            VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,'root','running',?)
        """, (vps_id, user.id, ct.id, image, os_label,
              ram, cpu, disk, cpu_name, ssh,
              SERVER_IP, host_port, root_pass, exp_at))
